...
const Nav = () => {
  ...
  return (
    <nav className='w-full h-24 flex items-center justify-between px-8 '>
        ...
        <div className={!value ? 'z-20 w-full min-h-screen absolute top-0 left-0 bg-white text-black flex items-center justify-between transition-all duration-300 ease-in md:hidden' : 'w-full min-h-screen absolute top-0 left-[-100%] bg-white text-black flex items-center justify-between duration-200 transition-all ease-in md:hidden'}>
            <ul className='gap-6 min-h-screen justify-center flex flex-col text-xl font-mono'>
                <li className='text-gray-700'><Link href={'/'}>HOME</Link></li>
                <li className='transition-all ease-in focus-within:text-red-800'><Link href={'/about'}>ABOUT</Link></li>
                <li className='transition-all ease-in focus-within:text-red-800'><Link href={'/education'}>EDUCATION</Link></li>
                <li className='transition-all ease-in focus-within:text-red-800'><Link href={'/projects'}>PROJECTS</Link></li>
                <li className='transition-all ease-in focus-within:text-red-800'><Link href={'/contact'}>CONTACT</Link></li>
            </ul>
